...
const zamaWorkerSrc = resolve('node_modules/@zama-fhe/sdk/dist/esm/relayer-sdk.worker.js');
const zamaBundleSrc = resolve('node_modules/@zama-fhe/relayer-sdk/bundle/relayer-sdk-js.umd.cjs');
...
const zamaBundleDir = resolve('node_modules/@zama-fhe/relayer-sdk/bundle');

/** MV3 CSP blocks remote script-src; load the relayer UMD from the extension package instead. */
...
/** WASM defaults to extension-root paths that 404 unless we bundle the binaries. */
...
export default defineConfig({
  ...
  plugins: [
    ...
    crx({ manifest }),
    {
      name: 'zama-worker-init-timeout',
      generateBundle(_options, bundle) {
        for (const chunk of Object.values(bundle)) {
          if (chunk.type !== 'chunk' || !chunk.fileName.includes('zama')) continue;
          // First WASM compile in MV3 can exceed the SDK default 60s INIT timeout.
          chunk.code = chunk.code.replace(
            /sendRequestTo\(([^,]+),([^,]+),([^,]+),\d+e\d+\)/g,
            'sendRequestTo($1,$2,$3,6e5)',
          );
        }
      },
    },
  ...
  build: {
    rollupOptions: {
      output: {
        manualChunks(id) {
          if (id.includes('node_modules/@zama-fhe/sdk')) {
            return 'zama';
          }
        },
      },
